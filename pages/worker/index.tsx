...
export default class WorkerDemo extends Component<{}, typeof defaultState> {
  constructor(props) {
    super(props)
  ...
  render() {
    return (
      <>
        ...
        <button type="button" onClick={this.asyncCalculate.bind(this)}>
          async Calculate
        </button>
        <button type="button" onClick={this.syncCalculate.bind(this)}>
          sync Calculate
        </button>
        <p>the result is: {this.state.result}</p>
        <button onClick={() => alert('this is a test message')}>test blocking</button>

        <p>当 num &gt; 44 时，采用 sync 方式明显卡顿，worker 异步明显不卡顿</p>
      </>
    )
  }
}
